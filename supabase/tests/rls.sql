...
set local role anon;
select throws_ok($$ select public.accept_interest('00000000-0000-0000-0000-000000000000') $$, 'anonymous callers cannot execute interest acceptance');
select throws_ok($$ select public.request_visit('00000000-0000-0000-0000-000000000000', now() + interval '2 days') $$, 'anonymous callers cannot execute visit scheduling');

set local role authenticated;
select set_config('request.jwt.claim.sub', '11111111-1111-1111-1111-111111111111', true);
select throws_ok($$ update public.profiles set phone_verified = true where id = auth.uid() $$, 'user cannot self-verify a phone number');
select lives_ok($$ update public.profiles set last_active_role = 'tenant' where id = auth.uid() $$, 'user can select an assigned active role');
...
select set_config('request.jwt.claim.sub', '11111111-1111-1111-1111-111111111111', true);
select lives_ok($$ select public.request_visit((select id from public.interests limit 1), now() + interval '2 days') $$, 'tenant can request a visit after a match');
select is((select status::text from public.visits limit 1), 'REQUESTED', 'new visit is requested');

select set_config('request.jwt.claim.sub', '22222222-2222-2222-2222-222222222222', true);
select lives_ok($$ select public.cancel_visit((select id from public.visits limit 1)) $$, 'listing manager can cancel a visit');
select is((select status::text from public.visits limit 1), 'CANCELLED', 'cancelled visit is persisted');
