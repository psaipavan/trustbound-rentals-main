...
export function BoundScoreBadge({ score, className }: { score: number; className?: string }) {
  return (
    <span
      ...
      title="Demo Bricxley Score — prototype value"
    >
      <span className="text-primary">{score}</span>
      <span className="opacity-70">/ 100</span>
      <span className="opacity-70">Demo Bricxley Score</span>
    </span>
  );
}
  ...
  return (
    <div className="surface-card p-5">
      ...
      <p className="mt-4 flex gap-2 rounded-lg bg-muted p-3 text-xs text-muted-foreground">
        ...
        Prototype value. Bricxley Score becomes a live, audited signal once backend verification is
